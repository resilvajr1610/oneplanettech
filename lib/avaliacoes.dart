...
class Avaliacoes extends StatefulWidget {
  ...
  Avaliacoes(this.usuario, this.controle, this.horariotrabalho, this.alunodoc);
...
class _AvaliacoesState extends State<Avaliacoes> {
  ...
  void initState() {
    ...
    Pesquisa().sendAnalyticsEvent(tela: Nomes().avaliacoesescola);
  }
                          ...
                          return ListView(
                            children: snapshot.data!.docs.map((doc) {
                              return Layout().itemCardapio(
                                  doc, widget.controle,  widget.usuario, 0.4, 'avaliacoes', context);
                            }).toList(),
                          );
                      }
                    },
  ...
  void parasalvar(BuildContext context) {
    Navigator.push(
        context, MaterialPageRoute(builder: (context) => AvaliacoesAdd(widget.usuario)));
  }
