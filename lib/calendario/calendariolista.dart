...
import 'package:scalifra/calendario/calendariolistaeventos.dart';
...
class CalendarioLista extends StatefulWidget {
  final DocumentSnapshot usuario, aluno;
  final bool controle;

  CalendarioLista(this.usuario, this.aluno, this.controle);

  @override
  _CalendarioListaState createState() => _CalendarioListaState();
}

class _CalendarioListaState extends State<CalendarioLista> {
  ...
  void initState() {
    ...
    Pesquisa().sendAnalyticsEvent(tela: Nomes().calendariobanco);
  }
      ...
      appBar: Layout().appbarcombotaosimples((context) {
        Pesquisa().irpara(
            CalendarioListaEventos(
                widget.usuario, widget.aluno, widget.controle),
            context);
      }, 'Calendário', "Lista Eventos", context),
      body: Row(
        children: [
